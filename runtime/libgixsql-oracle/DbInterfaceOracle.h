...
private:
	...
	int _odpi_exec_params(std::shared_ptr<ICursor> crsr, std::string query, int nParams, const std::vector<CobolVarType>& paramTypes, const std::vector<std::string>& paramValues, const std::vector<unsigned long>& paramLengths, const std::vector<int>& paramFormats, std::shared_ptr<OdpiStatementData> prep_stmt = nullptr);
